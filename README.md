...
> Note: Do **not** merge the contents of all the zips into one. Each downloaded zip file should expand to its own folder. All these folders should then be moved into `./in`. You should have folders with names like `72157721305918252_32dfb3965e35_part1` and `data-download-19`.
...
* Flickr `fave` -> Photos `Favorite` (Heart icon)
...
> Note: All albums are created in a single folder called "Flickr Photos". (See Configuration below.)
...
In my (limited) experience, this seems to happen when a Flickr photo was downloaded, modified, and re-uploaded as a new photo. In all my cases it was the *last* id in the filename that was correct. So the script makes this choice. But it always warns when it does this so you can inspect the files and see for sure. This rule worked for all my data but if it doesn't for you you may need to rename the offending photo file and re-verify, or modify the code.

### Error: ... photos have a photo file but no meta file

This means there is an image file in the data download folders that is not referenced by any metadata file. This probably means you missed one or more of the "Account Data" downloads.
...
* `default_keywords`: A keyword added to every imported photo.
